...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Validasi Input
    if (empty($data['meeting_name']) || empty($data['meeting_date']) || empty($data['meeting_time']) || empty($data['location']) || empty($data['inviter'])) {
        sendJSONResponse(['success' => false, 'message' => 'Semua kolom wajib diisi.'], 400);
    }

    try {
        $sql = "INSERT INTO meetings (meeting_name, meeting_date, meeting_time, location, agenda, inviter) 
                VALUES (:name, :date, :time, :location, :agenda, :inviter)";
